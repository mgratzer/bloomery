...
2. If yes, help them set it up:
   - The git repo is already initialized with a commit per step — they can run `git log --oneline` to see their full build history
...
3. Suggest they share the link with colleagues — it's a great conversation starter about how agents work under the hood. Anyone can clone it, set their API key, and try it out. The commit history shows the incremental build process step by step.
